...
// :p
if (!isset($_POST['send_tip']) AND !isset($_POST['send_tap']) AND !isset($_POST['ID_TIP']) AND !isset($_POST['modif_tiptap']) AND !isset($_POST['ID']) AND !isset($_POST['gr'])) {
	// stats;
	$nbMsg = countMsg()[0];
	...
	if (isset($_SESSION['token']) AND isset($_POST['token']) AND !empty($_SESSION['token']) AND !empty($_POST['token'])) { // form using POST;
		if ($_SESSION['token'] == $_POST['token'] OR $_SESSION['token'] == $_GET['token']) {
	...
	elseif (isset($_SESSION['token']) AND isset($_GET['token']) AND !empty($_SESSION['token']) AND !empty($_GET['token'])) { // form using GET;
		// Change theme & colors;
		if (isset($_GET['choose_color']) AND is_numeric($_GET['choose_color'])) {
